...
"#;
    ...
    for (read, (name, _evidence)) in reads.iter().zip(expected) {
        ...
        assert_eq!(
            &source[read.span.byte_start..read.span.byte_end],
            read.name
        );
        assert!(read.prefix.is_none());
        assert!(read.enclosing_symbol_id.as_deref().is_some_and(|symbol_id| {
            symbol_id.ends_with("::function:run")
        }));
    }
    ...
    assert!(reads.iter().all(|read| !write_offsets.contains(&read.span.byte_start)));
    ...
    let closure_body = source
        ...
        + "consume(".len();
    assert!(reads.iter().all(|read| read.span.byte_start != closure_body));
    ...
    assert!(encoded
        ...
        .iter()
        .any(|value| value.get("kind").and_then(serde_json::Value::as_str)
            == Some("variable_read")));
}
