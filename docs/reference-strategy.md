...
| lexical bindings and unqualified variable access | normative operator semantics plus heuristic scope intervals | callable and supported parenthesized closure parameters, block and supported braced loop/catch locals, reads, plain writes, and paired compound/increment read-write facts implemented | single-statement/collection and pattern/multi-declarator loops, existing-variable `for-in`, member/index writes, destructuring, and initializer ordering remain deferred |
