#!/usr/bin/env python3
"""Run the reviewed dependency-fingerprint patch with one disambiguated snapshot anchor."""
...
with tempfile.TemporaryDirectory() as directory:
    recovered = Path(directory) / "dependency_fingerprint_apply.py"
    recovered.write_text(source, encoding="utf-8")
    namespace = {"__name__": "__main__", "__file__": str(recovered)}
    exec(compile(source, str(recovered), "exec"), namespace)
