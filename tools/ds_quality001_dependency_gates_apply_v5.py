#!/usr/bin/env python3
"""Apply the reviewed DS-QUALITY-001 dependency gate with final hygiene fixes."""
...
""",
    """3. Added focused policy tests for missing metadata, expired exceptions, native-ignore drift, and a valid
   reviewed unused-dependency exception. The initial exception registry is empty.
4. **P1 fixed:** `dartscope-parse` declared `serde` directly without using it. The dependency was removed
   rather than hidden behind a cargo-machete exception.
...
crate-local use. It was removed instead of allowlisted; exceptions are reserved for reviewed false positives.
...
- Removed an unused direct `serde` dependency from `dartscope-parse` instead of suppressing the
...
print("DS-QUALITY-001 dependency gate v5 slice applied")
